...
export const defaultSlashSuggestions: SuggestionOptions<SuggestionItem>["items"] =
    () => [
      ...
      },
      {
        ...
        searchTerms: ["image", "photo", "picture", "img", "url"],
        icon: ImageIcon,
        command: ({ editor, range }) => {
...
// ---------------------------------------------------------------------------
// CodeBlockView — inline language selector rendered inside each code block
// ---------------------------------------------------------------------------
const LANGUAGES = [
  "text", "bash", "c", "cpp", "css", "diff", "go", "graphql", "html",
  "java", "javascript", "json", "kotlin", "lua", "markdown", "php",
  "python", "ruby", "rust", "scala", "sql", "swift", "toml", "tsx",
  "typescript", "xml", "yaml",
];
...
const CodeBlockView = ({ node, updateAttributes }: {
  ...
  const language = (node.attrs as { language?: string }).language ?? "text";

  return (
      <NodeViewWrapper as="div" className="my-4 rounded-md border overflow-hidden">
        <div
            className="flex items-center gap-2 border-b bg-muted px-3 py-1.5"
            contentEditable={false}
        >
          <span className="text-xs text-muted-foreground font-mono select-none">language:</span>
          <select
              value={language}
              onChange={(e) => updateAttributes({ language: e.target.value })}
              onMouseDown={(e) => e.stopPropagation()}
              onClick={(e) => e.stopPropagation()}
              className="text-xs bg-background border border-border rounded px-2 py-0.5 text-foreground font-mono cursor-pointer focus:outline-none focus:ring-1 focus:ring-primary"
          >
            {LANGUAGES.map((lang) => (
        ...
        <pre className={cn(
            "m-0 rounded-none border-0 p-4 text-sm font-mono bg-background text-foreground overflow-x-auto",
...
export const EditorProvider = ({
  ...
  const defaultExtensions = [
    ...
    TableHeader.configure({
      HTMLAttributes: {
        class: cn(
            "relative box-border min-w-[1em] border bg-secondary-darker p-1 text-start align-top font-medium font-semibold text-muted-foreground"
        ),
      },
    }),
    TaskList.configure({
    ...
    Image.configure({
      HTMLAttributes: {
        class: cn("rounded-md max-w-full h-auto my-2"),
      },
      allowBase64: false,
    }),
  ];
      ...
      <TooltipProvider>
        <div
            className={cn(
                ...
                // Heading sizes, weights, and vertical margins
                "[&_h1]:mt-6 [&_h1]:mb-2 [&_h1]:text-4xl [&_h1]:font-bold [&_h1]:leading-tight",
                "[&_h2]:mt-5 [&_h2]:mb-2 [&_h2]:text-3xl [&_h2]:font-bold [&_h2]:leading-tight",
                "[&_h3]:mt-4 [&_h3]:mb-1 [&_h3]:text-2xl [&_h3]:font-semibold [&_h3]:leading-snug",
                "[&_h4]:mt-4 [&_h4]:mb-1 [&_h4]:text-xl [&_h4]:font-semibold [&_h4]:leading-snug",
                "[&_h5]:mt-3 [&_h5]:mb-1 [&_h5]:text-base [&_h5]:font-semibold [&_h5]:leading-normal",
                "[&_h6]:mt-3 [&_h6]:mb-1 [&_h6]:text-xs [&_h6]:font-semibold [&_h6]:text-muted-foreground [&_h6]:leading-normal",
            )}
        >
...
export const EditorFloatingMenu = ({
  ...
  return (
      <FloatingMenu
          className={cn("flex items-center rounded-lg bg-muted", className)}
          editor={editor ?? null}
          {...props}
      />
  );
...
export const EditorFloatingMenuImageButton = () => {
  const { editor } = useCurrentEditor();
...
export const EditorLinkSelector = ({
  ...
  const [url, setUrl] = useState<string>(existingHref);
